...
async function setup(safeURL, toBase64) {
    ...
    let secret;
    let wrappingKeyReturn;
    let wrappedKeyReturn;
    // Typings for secret.
    if (safeURL) {
...
// }
describe.each(variation)("CripToe test: #%# safeURL: %s, toBase64: %s", async (safeURL, toBase64) => {
    const { C, secret, longTestMessage, wrappingKey, wrappedKey } = await setup(safeURL, toBase64);
    ...
    describe.shuffle("Methods", () => {
        ...
        test("encrypt() should encrypt a string", async () => {
            expect(() => new CripToeTest()).toThrowError("Not encrypted yet. You must call the 'encrypt' method before calling this property.");
            expect(secret.key).toBeInstanceOf(CryptoKey);
            expect
                .soft(!!(secret.initVector instanceof Uint8Array) ||
                !!isBase64(secret.initVector) ||
                !!isBase64(CripToe.decodeUrlSafeBase64(secret.initVector)), `${secret.initVector} was falsey. So, not a Uint8Array nor did it pass the base64 test.`)
                .toBeTruthy();
            if (toBase64 || safeURL) {
                expect(typeof secret.cipher === "string").toBeTruthy();
                if (typeof secret.cipher === "string") {
                    expect(isBase64(CripToe.decodeUrlSafeBase64(secret.cipher))).toBeTruthy();
                }
            }
        });
        test.runIf(safeURL || toBase64)("Should create a wrapping key.", async () => {
            if (typeof wrappedKey === "string") {
                expect(wrappedKey, "Should be a string.").toBeTypeOf("string");
                if (safeURL) {
                    expect(isBase64(wrappedKey), "Wrapped Key should not be a base64 string").toBeFalsy();
                }
                else {
                ...
                expect(safeURL
                    ...
                    : isBase64(wrappedKey), "SafeURL decoded wrapped Key should be a base64 string").toBeTruthy();
                expect(wrappingKey, "Wrapping Key is not a string.").toBeTypeOf("string");
            }
        });
        test.todo("Should unwrap a key.", async () => {
            ...
            if (typeof wrappedKey === "string") {
                ...
                const wrappingKeyJWK = Buffer.from(wrappingKey, "base64url").toString("utf-8");
                ...
                expect(
                //@ts-ignore Accessing a private property for testing.
                await C2.decrypt(secret.cipher, C2._cripKey, C.initVector)).toBe(longTestMessage);
            }
        });
        test.todo("Should decrypt an encrypted string", async () => {
            const decrypted = await C.decrypt(secret.cipher, secret.key, secret.initVector);
...
while (iterations--) {
    describe(`URL Encoding test: ${iterations}`, async () => {
        ...
        test("Should return URL safe base64", () => {
            ...
            expect(testUrl.toString(), "Cipher is not in the URL.").toContain(secret.cipher);
            expect(testUrl.searchParams.get("k"), "Wrapped Key is not in the URL.").toContain(wrappedKey);
            expect(testUrl.searchParams.get("iv"), "Init Vector is not in the URL.").toContain(secret.initVector);
            expect
                .soft(testUrl.toString().length, "Resultant URL is too long.")
                .toBeLessThanOrEqual(2000);
        });
        test("Should decode safe URL back to base64", () => {
            expect(CripToe.decodeUrlSafeBase64(C.encrypted), "Not a base64 string.").toMatch(CripToe.decodeUrlSafeBase64(secret.cipher));
        });
    });
}
